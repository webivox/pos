<?php
class InventoryMasterUnitsQuery {
	
	private $tableName="inventory_units";
    ...
    public function get($unitId) {
        ...
        $row = $db->fetch("SELECT * FROM ".$this->tableName." WHERE unit_id='".$unitId."'");
	...
	public function data($unitId,$column)
	{
		global $db;
	...
	{
		global $db;
		$res = $db->fetchAll("SELECT unit_id FROM ".$this->tableName." WHERE unit_id='".$unitId."'");
		$count = count($res);
		return $count;
	}
...
$InventoryMasterUnitsQuery = new InventoryMasterUnitsQuery;
?>