...
class AccountsMasterPayeeQuery {
	
	private $tableName="accounts_payee";
    
    public function get($payeeId) {
		
        global $db;

        // Query to fetch all blogs
        $row = $db->fetch("SELECT * FROM ".$this->tableName." WHERE payee_id='".$payeeId."'");
	...
	public function data($payeeId,$column)
	{
		global $db;
		
		$res = $db->fetch("SELECT ".$column." FROM ".$this->tableName." WHERE payee_id='".$payeeId."'");
	...
	public function has($payeeId)
	{
		global $db;
		$res = $db->fetchAll("SELECT payee_id FROM ".$this->tableName." WHERE payee_id='".$payeeId."'");
		$count = count($res);
		return $count;
	}
...
$AccountsMasterPayeeQuery = new AccountsMasterPayeeQuery;
?>