...
class InventoryRTransferConnector {
	...
	public function view()
	{
		...
		if($firewallCls->verifyUser())
		{
			...
			if($db->request('search_location_from')!==''){ $search_location_from=$db->request('search_location_from'); }
			else{ $search_location_from=''; }
			
			if($db->request('search_location_to')!==''){ $search_location_to=$db->request('search_location_to'); }
