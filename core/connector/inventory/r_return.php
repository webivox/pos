...
class InventoryRReturnConnector {
    ...
    public function index() {
		...
		if($firewallCls->verifyUser())
		{
			...
			$data['titleTag'] 	= 'Purchase Return Report | '.$defCls->master('companyName');
			$data['companyName'] 	= $defCls->master('companyName');
			$data['logo'] 			= _UPLOADS.$defCls->master('logo');
			
			$data['view_url'] = $defCls->genURL('inventory/r_return/view/');
			...
			require_once _HTML."inventory/r_return.php";
	...
	public function view()
	{
		...
		if($firewallCls->verifyUser())
		{
			...
			$data['title_tag'] = 'Inventory Return Note Report | '.$dateCls->todayDate('d-m-Y H:i:s').' | '.$data['companyName'];
			...
			$sql=" WHERE inventory_return_notes.return_note_id!=0";
			
			if($search_date_from){ $sql.=" AND DATE(inventory_return_notes.added_date)>='".$dateCls->dateToDB($search_date_from)."'"; }
			if($search_date_to){ $sql.=" AND DATE(inventory_return_notes.added_date)<='".$dateCls->dateToDB($search_date_to)."'"; }
			if($search_location){ $sql.=" AND inventory_return_notes.location_id='".$search_location."'"; }
			if($search_supplier){ $sql.=" AND inventory_return_notes.supplier_id='".$search_supplier."'"; }
			if($search_user){ $sql.=" AND inventory_return_notes.user_id='".$search_user."'"; }
			...
			$sql.=" ORDER BY inventory_return_notes.return_note_id DESC";
			
			/////
			
			$getRN = $db->fetchAll("SELECT 
			
										inventory_return_note_items.return_note_item_id AS `irni_return_note_item_id`,
										inventory_return_note_items.return_note_id AS `irni_return_note_id`,
										inventory_return_note_items.item_id AS `irni_item_id`,
										inventory_return_note_items.qty AS `irni_qty`,
										inventory_return_note_items.price AS `irni_price`,
										inventory_return_note_items.total AS `irni_total`,

										
										inventory_return_notes.return_note_id AS `irn_return_note_id`,
										inventory_return_notes.location_id AS `irn_location_id`,
										inventory_return_notes.supplier_id AS `irn_supplier_id`,
										inventory_return_notes.user_id AS `irn_user_id`,
										inventory_return_notes.rn_no AS `irn_rn_no`,
										inventory_return_notes.added_date AS `irn_added_date`,
										inventory_return_notes.total_value AS `irn_total_value`,
										inventory_return_notes.no_of_items AS `irn_no_of_items`,
										inventory_return_notes.no_of_qty AS `irn_no_of_qty`,
										...
										FROM 
											inventory_return_note_items
										INNER JOIN 
											inventory_return_notes ON inventory_return_note_items.return_note_id = inventory_return_notes.return_note_id
										INNER JOIN 
											inventory_items ON inventory_return_note_items.item_id = inventory_items.item_id
			...
			foreach($getRN as $cat)
			{
				...
				if ($currentRNId !== $cat['irn_return_note_id']) {
					...
					$data['rows'][] = array(
											...
											'rnh_row' => true,
											'rn_item_row' => true,
											'rn_no' => $defCls->docNo('RETN-',$cat['irn_return_note_id']),
					...
					$currentRNId = $cat['irn_return_note_id'];
			...
			$this_required_file = _HTML.'inventory/r_return_view.php';
