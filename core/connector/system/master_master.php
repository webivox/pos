...
class SystemMasterMasterConnector {
    ...
    public function index() {
		...
		{
			if($sessionCls->load('signedUserId')==1)
			{
				$data = [];
		...
		if($firewallCls->verifyUser())
		{
			...
			if($sessionCls->load('signedUserId')==1)
			{
				...
				if(($_SERVER['REQUEST_METHOD'] == 'POST'))
				{
					...
					if(strlen($data['key'])<1){ $error_msg[]="Key must be minimum 1 letters"; $error_no++; }
					if($countMasterByKey){ $error_msg[]="The key already exists"; $error_no++; }
					if(strlen($data['values'])<1){ $error_msg[]="Value must be minimum 1 letters"; $error_no++; }
