...
class CustomersRCreditnotesConnector {
    ...
    public function index() {
		...
		if($firewallCls->verifyUser())
		{
			...
			$data['titleTag'] 	= 'Customer Credit Note Report | '.$defCls->master('companyName');
			$data['companyName'] 	= $defCls->master('companyName');
			$data['logo'] 			= _UPLOADS.$defCls->master('logo');
			
			$data['view_url'] = $defCls->genURL('customers/r_creditnotes/view/');
			...
			require_once _HTML."customers/r_creditnotes.php";
	...
	public function view()
	{
		...
		if($firewallCls->verifyUser())
		{
			...
			$data['title_tag'] = 'Customer Credit Note Report | '.$dateCls->todayDate('d-m-Y H:i:s').' | '.$data['companyName'];
			...
			$getRows = $CustomersTransactionsCreditnotesQuery->gets($sql);
			...
			$this_required_file = _HTML.'customers/r_creditnotes_view.php';
