...
class CustomersRSettlementsConnector {
    ...
    public function index() {
		...
		if($firewallCls->verifyUser())
		{
			...
			$data['titleTag'] 	= 'Customer Settlement Report | '.$defCls->master('companyName');
			$data['companyName'] 	= $defCls->master('companyName');
			$data['logo'] 			= _UPLOADS.$defCls->master('logo');
			
			$data['view_url'] = $defCls->genURL('customers/r_settlements/view/');
			...
			require_once _HTML."customers/r_settlements.php";
	...
	public function view()
	{
		...
		if($firewallCls->verifyUser())
		{
			...
			$data['title_tag'] = 'Customer Settlement Report | '.$dateCls->todayDate('d-m-Y H:i:s').' | '.$data['companyName'];
			...
			$getRows = $CustomersTransactionsSettlementsQuery->gets($sql);
			...
			foreach($getRows as $cat)
			{
				...
				$data['rows'][] = array(
										'no' => $defCls->docNo('CSETT-',$cat['settlement_id']),
			...
			$this_required_file = _HTML.'customers/r_settlements_view.php';
